...
function Invoke-NativeCommand {
    ...
    if (-not $IgnoreExitCode -and $LASTEXITCODE -and $LASTEXITCODE -ne 0) {
...
function Install-WithWinget {
    ...
    if (Test-Command "winget") {
        ...
        Invoke-NativeCommand -FilePath "winget" -Arguments @("install", "--id", $PackageId, "--accept-source-agreements", "--accept-package-agreements", "--silent", "-e") -LastLineOnly -IgnoreExitCode
        # Refresh PATH
...
function Build-BitNet {
    ...
    if (-not (Test-Path (Join-Path $BITNET_DIR ".git"))) {
        ...
        Invoke-NativeCommand -FilePath "git" -Arguments @("pull") -LastLineOnly -IgnoreExitCode
        Pop-Location
    }

    # Ensure submodules are initialized (critical: setup_env.py needs 3rdparty/llama.cpp)
    Write-Info "Updating git submodules..."
    Push-Location $BITNET_DIR
    Invoke-NativeCommand -FilePath "git" -Arguments @("submodule", "update", "--init", "--recursive") -LastLineOnly
    Pop-Location

    $venvPython = Join-Path $VENV_DIR "Scripts\python.exe"
    $pipExe = Join-Path $VENV_DIR "Scripts\pip.exe"

    # Pre-install gguf-py so setup_env.py's internal pip install finds it.
    # Try the submodule path first; fall back to PyPI if it's broken.
    $ggufPy = Join-Path $BITNET_DIR "3rdparty\llama.cpp\gguf-py"
    if (Test-Path (Join-Path $ggufPy "setup.py")) {
        Write-Info "Installing gguf-py from submodule..."
