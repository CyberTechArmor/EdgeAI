...
function Build-BitNet {
    ...
    if (Test-Path $ggufPy) {
        ...
        & $pipExe install -e $ggufPy 2>&1 | Select-Object -Last 1 | ForEach-Object { Write-Info $_ }
    }
    $bitnetReqs = Join-Path $BITNET_DIR "requirements.txt"
    if (Test-Path $bitnetReqs) {
        & $pipExe install -r $bitnetReqs 2>&1 | Select-Object -Last 1 | ForEach-Object { Write-Info $_ }
    }
    & $pipExe install huggingface_hub 2>&1 | Select-Object -Last 1 | ForEach-Object { Write-Info $_ }
    ...
    if (Test-Path $setupScript) {
        ...
        Push-Location $BITNET_DIR
        & $venvPython setup_env.py --hf-repo $FALCON_HF_MODEL -q i2_s 2>&1 | ForEach-Object {
            $line = $_.ToString()
            # Show meaningful lines, suppress noise
            if ($line -match '(Downloading|Converting|Compiling|Building|cmake|error|warning|100%)') {
                Write-Info $line
            }
        }
        Pop-Location
        ...
        if ($clangPath) {
            ...
            cmake .. -T ClangCL -DBITNET_X86_TL2=OFF -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ 2>&1 | ForEach-Object { Write-Info $_ }
        } else {
        ...
        cmake --build . --config Release -j $threadCount 2>&1 | ForEach-Object { Write-Info $_ }
