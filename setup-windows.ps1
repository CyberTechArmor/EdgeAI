...
function Install-LLVM {
    # LLVM's NSIS installer often opens a GUI even with winget, so download and
    # run with /S (NSIS silent flag) directly. Also set ADD_TO_PATH.
    $llvmVersion = "18.1.8"
    $llvmUrl = "https://github.com/llvm/llvm-project/releases/download/llvmorg-$llvmVersion/LLVM-$llvmVersion-win64.exe"
    $installerPath = Join-Path $env:TEMP "LLVM-installer.exe"

    Write-Info "Downloading LLVM $llvmVersion (this is ~350 MB)..."
    Invoke-WebRequest -Uri $llvmUrl -OutFile $installerPath -UseBasicParsing

    Write-Info "Installing LLVM silently (this may take a minute)..."
    # /S = NSIS silent, /D= sets install dir (no space after /D=)
    $llvmInstallDir = "$env:ProgramFiles\LLVM"
    Start-Process -FilePath $installerPath -ArgumentList "/S", "/D=$llvmInstallDir" -Wait -NoNewWindow

    Remove-Item $installerPath -ErrorAction SilentlyContinue

    # Add LLVM to PATH for this session and permanently
    $llvmBin = Join-Path $llvmInstallDir "bin"
    if (Test-Path $llvmBin) {
        $env:Path = "$llvmBin;$env:Path"
        # Also add to system PATH permanently so future sessions find it
